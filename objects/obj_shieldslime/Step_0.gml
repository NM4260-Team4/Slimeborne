...
switch state {
	case SLIMEENEMY_STATE.ROAM:
		if (inner_state == 0) {
			...
			if (abs(x - obj_player.x) < 800 and abs(y - obj_player.y) < 50 and sees_player()) {
	...
	case SLIMEENEMY_STATE.TARGETING:
		if (inner_state == 0) {
			...
			if (abs(x - obj_player.x) >= 800 or abs(y - obj_player.y) > 50 or not sees_player()) {
	...
	case SLIMEENEMY_STATE.HIT:
		if (inner_state == 0) {
			...
			inner_state = 1;
			move_speed = 0;
			audio_stop_sound(snd_shieldslime_attack);
			audio_play_sound(snd_shieldslime_hit, 10, false);
		} else if (inner_state == 1) {
			move_x = 0;
		} else {
			is_hit = false;
	...
	case SLIMEENEMY_STATE.DEATH:
		if (inner_state == 0) {
			...
			move_speed = 0;
			audio_stop_sound(snd_shieldslime_attack);
			audio_stop_sound(snd_shieldslime_hit);
